...
public class RandomAgent : Agent
{
	...
	public override void OnEpisodeBegin() {
		...
		while (distanceAtoB < minimumDistance || distanceAtoC < minimumDistance || distanceBtoC < minimumDistance) {
			InitSpeakers();
	...
	public override void OnActionReceived(ActionBuffers actions) {
		...
		controlSignal.x = Random.Range(-1.0f, 1.0f);
		controlSignal.z = Random.Range(-1.0f, 1.0f);
		rBody.AddForce(controlSignal * forceMultiplier);
		
		// Rewards
		float distanceToTarget = Vector3.Distance(this.transform.localPosition, targetSpeaker.transform.localPosition);
		// Reached target
		...
		for (int i = 0; i < speakers.Length; ++i) {
			float distanceToSpeaker = Vector3.Distance(this.transform.localPosition, speakers[i].transform.localPosition);
